...
const RelatedDoctors = ({speciality, docId}) => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 md:mx-10 text-gray-900'>
            ...
            <div onClick={()=>{navigate(`/appointment/${item._id}`), scrollTo(0,0)}} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                ...
                <div className='p-4'>
                    <div className={`flex items-center gap-2 text-sm text-center ${item.available ? "bg-green-500" : "bg-gray-500"}`}>
                        <p className={`w-2 h-2 ${item.available ? "text-green-500" : "text-gray-500"} rounded-full`}></p><p>{item.available ? "Available" : "Not Available"}</p>
                    </div>
      ...
      {/* <button onClick={()=> {navigate("/doctor"); scrollTo(0,0)}} className='text-gray-600 bg-blue-50 px-12 py-3 rounded-full mt-10'>more</button> */}
    </div>
  )
}
